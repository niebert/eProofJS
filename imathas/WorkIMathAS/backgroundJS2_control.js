...
for ($i=1..$maxSteps) {
    ...
    for ($so=0..$SolutionCount) {
           ...
           if ($CorrectProofStep == $QuestionStepID[$i-1]) {
                 $DebugHTML .="<br/>([".$i."]  FOUND: '".$CorrectProofStep."'=[CorrectID]=[QuestionID]'".$QuestionStepID[$i-1]."')" 
                 //------------------------------------------
                 //--- Check Score of this Solution Step ----
                 //------------------------------------------
                 // $InterSection = intersectarrays($CorrectPreconditions[$so],$StudentsPrecondition[$i])
                 $InterSection = intersectarrays($CorrectPreconditions[$so],$QuestionPreconditionArray[$i-1])
                 ...
                 $Temp_NotNecessary = diffarrays($QuestionPreconditionArray[$i-1],$CorrectPreconditions[$so])
                 $Temp_Empty = array(" ","")
                 ...
                 //------------------------------------
                 // diffarrays(array1,array2): Returns all elements in array1 that are not also in array2
                 $Temp_Missing = diffarrays($CorrectPreconArray[$so],$QuestionPreconditionArray[$i-1])
...
//----------------------
$vAverageScore = 0.0
if (($Step_Input_Count > 0) && ($MinimalProofSteps>0)) {
   $vAverageScore = (sumarray($vSCORE)/$Step_Input_Count)
   if ($Step_Input_Count > $MinimalProofSteps) {
...
$vHTML .= $vRULE
for ($i=1..$maxSteps) {
    $vConnectionTAG = $QuestionConnectionTAG[$i]
...
$FeedbackLine .= $Def_TD_I.$Step_Input_Count." ".$STR_ProofSteps." --- Minimal $STR_ProofSteps=".$MinimalProofSteps."  "
