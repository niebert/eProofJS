...
for ($i=0..$maxSteps) {
    for ($ai=0..2) {
       ...
       if ($max_false>0) {
          $False_Index = rands(1,$max_false,$false_count)
       } else {
          $False_Index = array()
       }
...
for ($i=$iBegin..$maxSteps) {
   ...
   if ($i == $maxSteps) {
     ...
     $form_input_element = "'inJUSTIFICATION".$vQID.$vNr."'"
     for ($c=0..$max_c) {
...
for ($i=0..$maxSteps) {
     $vArrayID0  = $Possible_ConnectionsID[$i]
     $vArrayID1  = $Possible_ProofStepsID[$i]
     $vArrayID2  = $Possible_PreconditionsID[$i]
     $vHTML .= "<li>"
     $vHTML .= "$STR_possible $STR_Connections [".arraytolist($vArrayID0)."]" 
     $vHTML .= "$STR_possible $STR_ProofSteps [".arraytolist($vArrayID1)."]" 
     $vHTML .= "$STR_possible $STR_Justifications [".arraytolist($vArrayID2)."]" 
     $vHTML .= "</li>"
}
$vHTML .= "</ol>"
...
for ($vNr=1..$vCount) {
  ...
  //-----------------------------------------------------
  if ($vNr == $imathSTEPCOUNT) {
     $vContent .= "\n  <DIV class='newSTUDENTANSWER".$vQID."' id='newSTUDENTANSWER".$vQID."' step='NEW' >"
     //----Begin: EDIT_STUDENTANSWER---------------------
     $vContent.="\n    <DIV class='neweditNEWSTUDENTANSWER".$vQID."' id='neweditSTUDENTANSWER".$vQID."'>"
     //-----POSITION---------
     $vName = "newPOSITION".$vQID
     $vContent .= "\n<SELECT step='".$vNr."' class='".$vName."' id='".$vName.$vNr."'>"
     $vContent .= "\n<OPTION VALUE='".$vPos."'>".$vPos."</OPTION>"
     $vContent .= "\n</SELECT>";
     //-----[+]-Button---------
     $vContent .= "\n<input type='button' class='bUseStep".$vQID."' step='".$vNr."'  id='bUseStep".$vQID.$vNr."' value=' + ' onclick=\"appendStep(".$vNr.")\" style=\"display:none\" />" 
     //---CONNECTION-----------
     $vName = "newCONNECTION".$vQID
     $vContent .= "<SELECT class='".$vName."' step='NEW' id='".$vName."'  onchange=\"updateStep(".$vNr.")\" >"
     $max_con = count($StepConnection)-1
     for ($con=0..$max_con) {
       $vContent .= "\n<option value='".$con."'>".$StepConnection[$con]. "</option>"
     }
     ...
     //----STEP DEFINITION ID-----
     $vName = "newSTEP".$vQID
     $vContent .= "\n<SELECT step='NEW' class='".$vName."' id='".$vName."' onchange=\"moveStepID(".$vNr.",this.value,".$vCount.")\" >"
     $vContent .= "\n<option class='OPTION_UNDEFINED' value='???' selected='selected'>???</option>"
     for ($i=1..$vCount) {
       $iID = $AllID[$i]
       $vContent .= "\n<option class='OPTION_".$iID."' value='".$iID."'>".$iID."</option>"
     }
     $vContent .= "\n</SELECT>"
     //----[ID] inSTEPID------
     $vContent .= "\n<input class=\"newinSTEPID".$vQID."\" type='text' size='3'  step='".$vNr."' id='inSTEPID".$vQID.$vNr."'  value=\"".$Q_StepID."\" style='display:none' />"
     //------JUSTIFICATION----------------------
     //$vContent .= "\n<input type='button' class='newbJUSTIFICATION".$vQID."' step='NEW'  id='newbJUSTIFICATION".$vQID."' value='". $STR_Justifications."' onclick=\"toggleJustification(".$vNr.")\" />"
     //$vContent .= "\n<input type='text' size='15' class='newinJUSTIFICATION".$vQID."' step='".$vNr."' id='newinJUSTIFICATION".$vQID."' value='' onchange=\"changeEditJustification(".$vNr.")\" ".$vHideShowCount."/>"
     //$vContent .= "\n<input class=\"STEP".$vQID."\" type='text' size='3'  value='".$vNr."' id='STEP".$vQID.$vNr."' style='display:none' />" // style='display:none' style='visibility:hidden'  
    //-----------------------------------------------------
     $vContent.="    </DIV>\n" // End: neweditSTUDENTANSWER
     $vContent.="  </DIV>\n" // End: newSTUDENTANSWER
