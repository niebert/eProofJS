...
function append_template_EProof__SID__(pAnswerHash,pInnerHash,pStep) {
	...
	if (cln) {
		...
		this.writeInnerHTML4Root(cln,"outSTEPDEF"+this.aQID,this.getStepRAW(vID));
	} else {
		...
		if (vJustString != "") {
			...
			while (i != vJustArray.length) {
				...
				if (this.aAllID2RAW[vID]) {
					...
					vJustHTML += "["+this.aMappedID[vID]+"] "+this.getStepRAW(vID);
...
function init_EProof__SID__() {
	...
	this.aUsedDOM     = document.getElementById(+this.aQID); //set DOM in this.preProcess()
	this.aUsedSteps   = null;
	this.aUnusedID    = "UNUSEDSTEPS"; // append QID load() and
	this.aUnusedDOM   = document.getElementById("UNUSEDSTEPS"+this.aQID);; //set DOM in this.preProcess()
...
function load_EProof__SID__(pQID,pThisQ,pRootDOM,pRootID,pMode) {
	...
	if (pMode) {
		this.aMode = pMode;
		if (this.aMode == "ELECTRON") {
			this.aElectron = "1";
			this.aMode = "AUTHORING";
			this.appendElectronMethods = appendElectronMethods_EProof__SID__;
			this.appendElectronMethods();
		};
...
function postProcess_EProof__SID__() {
	...
	console.log("Theorem_Title: "+vHash["THEOREMTITLE"]);
...
//#################################################################
// pChildSA is a Child of StudentAnswer
function updateAllMath_EProof__SID__() {
	var i = 0;
	var vID = "";
	var vValue = "";
	var vStep =0;
	while (i != this.aAllID.length) {
		vID = this.aAllID[i];
		vStep = this.getStep4ID(vID);
		vValue = this.decodeValue(this.aAllID2RAW[vID]);
		this.writeInnerHTML4Math("ID-"+this.aQID+"-"+vID,vValue);
		if (this.greater(vStep,0)) {
			this.writeInnerHTML4Math("outSTEPDEF"+this.aQID+vStep,vValue)
		};
		i++;
	};
};
...
function updateJustifications_EProof__SID__(pChildSA,pUpdateEdit) {
	...
	if (!vInputIDs) {
		...
		this.processMathNode(vOutputDisplay);
		if (pUpdateEdit) {
			...
			this.processMathNode(vOutputEdit);
		};
...
function updateTextarea_EProof__SID__(pChildSA) {
  var vStep = pChildSA.getAttribute("step");
	...
	console.log("updateTextArea() Step="+vStep+" ["+vID+"]");
  ...
  this.aAllID2RAW[vID] = this.encodeTextarea(vValue);
	...
	//----------------------------------------------------------------------
	//---EDIT.value: Update the Step Definition input type="text" one-liner
